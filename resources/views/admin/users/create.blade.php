...
@section('content')
    <div class="p-8 pt-20 w-full">
        <div class="p-8 bg-slate-800 rounded-lg border border-slate-700 w-full">

            <h1 class="text-4xl font-bold mb-6 text-white">Tambah User Baru</h1>
            <p class="text-sm text-slate-400 mb-6">Buat akun baru untuk Admin, Manager, atau Staff.</p>

            @if (session('error'))
                <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-rose-900/10" role="alert">
                    {{ session('error') }}
                </div>
            @endif

            @if ($errors->any())
                <div class="p-3 mb-4 text-sm rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-200">
