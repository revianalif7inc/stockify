...
@section('content')
    <div class="p-4 pt-4">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-white">Konfirmasi Barang Masuk</h1>
            <p class="text-slate-400">Periksa dan konfirmasi detail barang masuk berikut.</p>
        </div>
        ...
        <div class="rounded-lg border border-slate-700 overflow-hidden bg-slate-800/30 p-6">
            ...
            <form method="POST" action="{{ route('staff.stock.in.do_confirm', $movement->id) }}">
                @csrf
                <button
                    class="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium">✅
                    Konfirmasi</button>
                <a href="{{ route('staff.stock.in') }}"
                    class="ml-4 inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-200">←
                    Kembali</a>
            </form>
        </div>
    </div>
