...
@section('content')
    <div class="p-4 pt-4">
        <!-- Header Section -->
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-white mb-2">Barang Masuk</h1>
            <p class="text-slate-400">Catat penerimaan stok barang dari supplier</p>
        </div>

        <!-- Add Button -->
        <div class="mb-6">
            <button onclick="openAddModal()"
                class="text-white bg-emerald-600 hover:bg-emerald-700 font-medium rounded-lg text-sm px-5 py-2.5 transition">
        ...
        <x-table-card>
            <table class="w-full text-sm text-left">
                <thead class="text-xs uppercase font-semibold bg-slate-800 border-b border-slate-700">
                    ...
                    <tbody>
                        @php 
                            $movements = isset($products) ? \App\Models\StockMovement::where('type', 'in')->with('product.supplier', 'user')->latest('created_at')->get() : collect([]);
                        @endphp
                        @forelse($movements as $index => $movement)
                            <tr class="border-t border-slate-700 hover:bg-slate-900/50 transition">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-emerald-950/50 text-emerald-300 font-bold text-sm">{{ $movement->quantity }}</span>
                                </td>
                                <td class="px-6 py-4 text-slate-300">{{ $movement->product->supplier->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4">
                        ...
                        @empty
                            <tr class="border-t border-slate-700">
                                <td colspan="9" class="px-6 py-4 text-center text-slate-400">Tidak ada data barang masuk</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div id="addModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-slate-900 rounded-lg p-6 max-w-md w-full mx-4 border border-slate-700">
                <h3 id="modalTitle" class="text-xl font-bold text-white mb-4">Tambah Barang Masuk</h3>
                <form id="stockForm" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="supplier_id" id="supplierSelect" class="w-full px-4 py-2 rounded-lg text-sm border border-slate-600 bg-slate-800 text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
    ...
    <script>
        ...
        function openAddModal() {
            ...
            document.getElementById('stockForm').action = "{{ route('manager.stock.in.store') }}";
        ...
        function openEditModal(movementId) {
            ...
            const editUrl = `/manager/stock/in/${movementId}/edit`;
            ...
            .then(data => {
                ...
                document.getElementById('stockForm').action = `/manager/stock/in/${movementId}`;
        ...
        function deleteItem(movementId) {
            if (confirm('Apakah Anda yakin ingin menghapus transaksi ini? Stok akan direvert otomatis.')) {
                document.getElementById('deleteForm').action = `/manager/stock/in/${movementId}`;
